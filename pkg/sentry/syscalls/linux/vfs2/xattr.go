...
func getxattr(t *kernel.Task, args arch.SyscallArguments, shouldFollowFinalSymlink shouldFollowFinalSymlink) (uintptr, *kernel.SyscallControl, error) {
	...
	if len(value) > linux.XATTR_SIZE_MAX {
		return 0, nil, syserror.E2BIG
	}
	if err := copyOutXattrValue(t, valueAddr, size, value); err != nil {
		return 0, nil, err
	}
	return uintptr(len(value)), nil, nil
}
...
func copyOutXattrValue(t *kernel.Task, valueAddr usermem.Addr, size uint, value string) error {
	if size == 0 {
		return nil
	}
	if size > linux.XATTR_SIZE_MAX {
