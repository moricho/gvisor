...
//
// FIXME(jamieliu): Does this have an equivalent in Linux?
...
func Ppoll(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	if maskAddr != 0 {
		...
		var maskStruct marshalableSignalSet
		if _, err := maskStruct.CopyIn(t, maskAddr); err != nil {
			return 0, nil, err
		}
		mask := linux.SignalSet(maskStruct.data) &^ kernel.UnblockableSignals
...
func Pselect(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	if maskWithSizeAddr != 0 {
		maskAddr, maskSize, err := copyInSigSetWithSize(t, maskWithSizeAddr)
		if err != nil {
			return 0, nil, err
		}
...
func copyTimespecInToDuration(t *kernel.Task, timespecAddr usermem.Addr) (time.Duration, error) {
	...
	if timespecAddr != 0 {
		...
		_, err := timespec.CopyIn(t, timespecAddr)
		if err != nil {
			return 0, err
		}
		if !timespec.Valid() {
