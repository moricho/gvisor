...
func Getcwd(t *kernel.Task, args arch.SyscallArguments) (uintptr, *kernel.SyscallControl, error) {
	...
	// Write the pathname string.
	n, err := t.CopyOutBytes(addr, []byte(s))
	if err != nil {
		return 0, nil, err
	}

	// Write the NUL terminator.
	if _, err := t.MemoryManager().ZeroOut(t, addr+usermem.Addr(n), 1, usermem.IOOpts{
		AddressSpaceActive: true,
	}); err != nil {
		return 0, nil, err
	}

	return uintptr(n + 1), nil, nil
}
